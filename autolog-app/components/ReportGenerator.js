...
<title>Car Story Report — ${escapeHtml(vehicleName)}</title>
<style>
...
<div class="header">
  <h1>🚗 Car Story Vehicle Report</h1>
...
<div class="section">
  <h2>Issues</h2>
  <table>
    <tr><th>Date</th><th>Title</th><th>Severity</th><th>Status</th><th>Cost</th><th>Description</th></tr>
    ${issues.map(i => `<tr>
      <td>${formatDate(i.date)}</td>
      <td>${escapeHtml(i.title)}</td>
      <td><span style="display:inline-block;padding:2px 8px;border-radius:8px;font-size:11px;font-weight:600;background:${
        i.severity === 'critical' ? '#fee2e2;color:#991b1b' :
        i.severity === 'serious' ? '#fed7aa;color:#c2410c' :
        i.severity === 'moderate' ? '#fef3c7;color:#92400e' : '#dbeafe;color:#1e40af'
      }">${i.severity}</span></td>
      <td><span style="display:inline-block;padding:2px 8px;border-radius:8px;font-size:11px;font-weight:600;background:${
        i.status === 'resolved' ? '#dcfce7;color:#166534' :
        i.status === 'in_progress' ? '#fef3c7;color:#92400e' : '#fee2e2;color:#991b1b'
      }">${i.status === 'in_progress' ? 'in progress' : i.status}</span></td>
      <td class="cost">${i.cost ? formatCurrency(i.cost) : '—'}</td>
...
<div class="footer">
  Generated by Car Story — Your personal car maintenance companion<br>
  Photo count: ${images.length} | Issues: ${issues.length} | Health Score: ${healthScore != null ? healthScore + '%' : 'N/A'}
</div>
  ...
  // Share or print/export the report
  if (Platform.OS === 'web') {
    ...
    if (newWindow) {
      ...
      // Add PDF export and share functionality
      setTimeout(() => {
        // Try to use Web Share API first
        if (navigator.share && navigator.canShare) {
          try {
            navigator.share({
              title: `${vehicleName} — Vehicle Report`,
              text: `Vehicle maintenance report for ${vehicleName}`,
              url: newWindow.location.href,
            });
          } catch (error) {
            console.warn('Web Share API failed, falling back to print dialog');
            newWindow.print();
          }
        } else {
          // Fallback to print dialog (which includes save as PDF option)
          newWindow.print();
        }
      }, 500);
    }
